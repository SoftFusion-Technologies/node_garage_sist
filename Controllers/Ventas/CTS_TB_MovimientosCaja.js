 ...
 *
 * Descripción:
 * Este archivo (CTS_TB_MovimientosCaja.js) contiene controladores para manejar
 * operaciones CRUD sobre la tabla movimientos_caja.
 *
 * En esta versión se incorpora el campo 'origen' como opcional, para clasificar
 * los movimientos (venta, retiro de recaudación, gasto, ajuste, etc.).
 *
...
export const CR_MovimientoCaja_CTS = async (req, res) => {
  const {
    caja_id,
    tipo,
    origen, // ✅ ahora se admite, pero no es obligatorio
    descripcion,
    monto,
    fecha,
    referencia
  ...
  try {
    const nuevo = await MovimientosCajaModel.create({
      caja_id,
      tipo,
      origen: origen || null, // si no viene, queda NULL
      descripcion,
      monto,
      fecha,
      referencia
